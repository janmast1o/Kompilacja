...
class TypeChecker(NodeVisitor):
    ...
    def visit_Variable(self, node: AST.Variable):
        node_id = node.name
        node_id_type, add = self.visit(node_id)
        # print(node_id_type)
    ...
    def visit_RelationExpression(self, node: AST.RelationExpression):
        left_side_type, _ = self.visit(node.left)
        op = node.op
        right_side_type, _ = self.visit(node.right)

        result_type = dict_of_types[op][left_side_type][right_side_type]
        if result_type != "bool":
            raise Exception("Condition should be asserted as bool"
                            f"but {left_side_type} {op} {right_side_type} "
    ...
    def visit_MatrixFuncNode(self, node: AST.MatrixFuncNode):
        arg_type_x, _ = self.visit(node.arg_x)
        arg_type_y, _ = self.visit(node.arg_y)
        if arg_type_x != "int" or arg_type_y != "int":
            raise Exception("Matrix functions must be initialized with int,"
                            f"not {arg_type_x} and {arg_type_y}")
    ...
    def visit_ListOfPrintablesNode(self, node: AST.ListOfPrintablesNode):
        return None, None
